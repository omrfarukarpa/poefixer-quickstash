...
> Güncellendi: 2026-09-13. Kanıt: QuickStash.cpp, config/game/input/overlay/ui başlıkları, çözüm/proje dosyaları ve SDK imzaları. Oyun içi çalışma bu yenilemede gözlenmedi.

## Giriş ve sınırlar

Windows x64/C++20 DLL; `QuickStash.sln` ve `QuickStash.vcxproj`, MSVC v145. Tek eklenti derleme birimi `QuickStash.cpp`; diğer birimler vendored ImGui. Güncel kod sürümü 1.4.1. Kurallar `AGENTS.md`, yayın/kararlar `PROJECT_MEMORY.md` içindedir.
...
5. Highlight alanı bulunmadan TAKE hedefi oluşturulmaz. Alan bulunmuş ve filtre boşsa bütün uygun eşyalar seçilir. TAKE sayısı tıklama/yığın sayısıdır; ayrı miktar etiketi yığın toplamını verir.
...
- Mod metni yalnızca `readMods` açık ve filtre doluyken okunur. Liveness probe `ReadItemBaseTypeName`; sonra ReadItemMods/ReadItemAggregatedStats. Corrupted bilgisi mod sonucundan gelir.
