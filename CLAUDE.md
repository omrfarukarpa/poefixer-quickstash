...
## Work in progress — Withdraw ("pick up highlighted items from stash")

**Goal (community request):** a reverse of Transfer — Ctrl+click items in the OPEN stash back INTO the inventory, filtered (a search box emulating PoE's stash "highlight"). User chose the *filtered withdraw* variant.

**Status (2026-07-08): WITHDRAW SHIPPED in v1.2.0** (commit `98a2084`, GitHub release `v1.2.0`, DLL SHA-256 `8229109c4a2c7641aea76894c59d69ee6dbbc1314f55db3567becd55ad133f9d`). A "TAKE (N)" button sits above PoE's native "Highlight Items" box; it Ctrl+clicks the open stash tab's filter-matching items back into the bag through the existing `TransferState` (all Ctrl-hold / watchdog / foreground-abort / cursor-restore guarantees shared). The filter comes from reading PoE's Highlight box via the UI tree (`game/PoeHighlight.h`). Mod-text matching remains gated OFF (see below). Verified in-game by the maintainer.

**What withdraw filters on (VERIFIED against the SDK — corrects an earlier wrong note):**
- Filter is a case-insensitive substring matched against a per-item search text = `BaseTypeName` + `UniqueName` + `Path` + **every mod's `AffixName`, `Name`, and formatted stat line**. So "fire" DOES match a Ruby Ring via its "+X% to Fire Resistance" stat — PoE-highlight-like. Empty filter = take everything on the tab.
- **Mods ARE readable per item in principle** — via `InventoryService::read_item_mods_summary` + `enumerate_item_mods_by_entity`, wrapped as `ctx->Inventory.ReadItemMods(item.Address)` → `ItemMods{...}`; each `Mod` has `AffixName`/`StatKey`/`Value0/1`; `ctx->Inventory.FormatStat(StatKey, v0, v1)` renders the in-game stat line. **BUT: calling `ReadItemMods(item.Address)` CRASHED the host in-game (2026-07-08).** Almost certainly `item.Address` (the value in `InventoryItemAbi.address`) is NOT the entity address `read_item_mods_summary` expects for every item, and the host derefs bad memory (access violation — not catchable by try/catch). Mod matching is therefore GATED OFF behind `g_withdrawReadMods` (false) in `game/WithdrawPlanner.h`; `BuildItemSearchText` currently uses only `BaseTypeName`+`UniqueName`+`Path`. **Before re-enabling: confirm the correct entity address** (try the lower-risk `ReadItemBaseTypeName(item.Address)` first — if it returns the right name, the address is valid; if it also crashes, the address is wrong and mods need a different handle) and guard against no-mod item types.
- Also readable lazily off `item.Address` if the eager bulk strings are ever empty: `ReadItemBaseTypeName/ReadItemUniqueName/ReadItemPath` (not currently needed — the bulk fields were populated in-game 2026-07-08).
- **Cannot read PoE's native "Highlight Items" box** — `InventoryItemAbi` has no highlight/selected field. Our filter is a *separate* box (currently in DrawSettings); we compute the match set and draw our OWN cyan highlight overlay (`DrawWithdrawHighlights`).
- **Cannot read item render brightness/alpha/color** — the game's dim/brighten is a client render effect; `InventoryItemAbi` has no color/alpha and `Render` is world-only. Irrelevant now that we match mod text ourselves.

**Highlight click-point / rect resolution (the hybrid, ORDER MATTERS):** per item, PER-ITEM screen rect FIRST (`ScreenValid` + on-screen → `ScreenX/Y/W/H`), grid math ONLY as fallback (`Grid.GridScreenX/Y + slot*CellSize`). Getting this backwards drew currency-tab highlights as a stretched off-screen band, because a special tab's logical grid (e.g. 53x4) does NOT match its visual layout while its per-item rects are correct. Normal grid tabs have `ScreenValid=0` so they fall through to grid math; special tabs have real per-item rects and use them.

**Key findings (why this is non-trivial):**
- `InventoryService::GetAll()` enumerates **100+ inventories** at once: `MainInventory1`, every equipment slot (`BodyArmour1`, `Weapon1/2`, `Offhand1/2`, `Helm1`, `Amulet1`, `Ring1/2`, `Gloves1`, `Boots1`, `Belt1`, `Flask1`, `Trinket`), `Cursor1`, **and every stash tab you own**. Ids 1–13 are the fixed player slots; stash tabs are id 14+.
- **RESOLVED (in-game diagnostic, 2026-07-08): click-point resolution is per-tab-type and must be HYBRID.** Two paths, each proven on live data:
  - **Normal grid tab → GRID-centric** (the exact math Transfer already uses): `SlotCenterX/Y = Grid.GridScreenX/Y + (slot + 0.5)·CellSize`. Confirmed: a visible normal 12×12 tab reports `Grid.Valid=1`, on-screen grid `(23,169)` `cell=70`, correct click points — **but its per-item `ScreenValid=0`** (item-centric returns nothing for normal tabs).
  - **Special/affinity tab (currency/fragment/…) → ITEM-centric**: per-item `ScreenX/Y/W/H` + `ScreenValid`. Confirmed: a visible currency tab reports every item `ScreenValid` and on-screen (e.g. 37/37/37).
  - So the earlier "item-centric only" bet was **wrong for normal tabs**. Withdraw must, per item, use grid math when `Grid.Valid && GridOnScreen`, else fall back to the item screen center when `ScreenValid && ItemOnScreen`.
- **The TabletHelper (ExileCore2) worry does NOT apply to PoeFixer.** In ExileCore2 special tabs don't expose item rects and need a UI-tree reflection hack (`StashElement.IndexVisibleStash`); PoeFixer has no UI-tree access but doesn't need one — the host resolves the coordinates we need on both tab types.
- **Visible-tab isolation looks clean:** with one tab open, only that tab reports on-screen (grid-on-screen OR on-screen items). The old "many affinity tabs active at once" fear did not reproduce for non-empty tabs in testing — but keep verifying when multiple non-empty tabs could be laid out.

**Code (local, uncommitted, builds clean):**
- `game/WithdrawPlanner.h` — `ContainsCI`, `ItemMatchesFilter`, `ResolveItemClick` (the hybrid grid/item click resolver), `TabOnScreen`, `FindOpenStashAny` (finds the one on-screen non-player tab; excludes bag/equipment so withdraw can't hit the player's own inventory), `BuildWithdrawClicks`.
- `game/TransferPlanner.h` — added `ScreenPoint`; still holds `SlotCenterX/Y`, `BuildClickQueue`.
- `game/TransferState.h` — added `StartWithdraw(ctx, settings, vector<ScreenPoint>)` + `IsWithdrawMode()`; `m_useScreenPoints`/`m_screenQueue` drive absolute points (fixed at launch, not recomputed) while transfer keeps the live-grid recompute; `ActiveQueueSize()` unifies both.
- `overlay/TransferButtonOverlay.h` — `DrawOverlayButton` shared body; `DrawWithdrawButton(inv, settings, count)` renders **"TAKE (N)"** at `WithdrawButtonPos` (right of transfer). `DrawTransferProgress` takes a verb.
- `config/Settings.h` — `withdrawFilter` string (persisted).
- `QuickStash.cpp` — draws both buttons, single per-frame `Win32LeftClickOnRect` (shared static edge state), `StartWithdraw()`, `UpdateWithdrawCount()` (200 ms cached count for the button), `m_withdrawFilterBuf` synced with settings.
- `ui/InventoryDiagnostics.h` / `game/PanelDetector.h` — the diagnostics panel + `IsPlayerSlotName`/`GridOnScreen`/`ItemOnScreen`/`FindOpenStash` from the diagnostic phase, still present.

**Next steps when resuming:** (1) IN-GAME VERIFY: open bag + a stash tab, set the withdraw filter (e.g. `Ruby`), confirm the "TAKE (N)" count is right and clicking pulls exactly those items and NEVER the player's own bag; test a normal tab AND a special/affinity tab (currency). (2) Consider an in-game filter box near the button (currently the filter lives in DrawSettings) — optional UX polish. (3) When happy, bump version + release per the publishing workflow.
