...
**Status as of last session (2026-07-08): WITHDRAW IMPLEMENTED (local, uncommitted, builds clean); NOT yet released.** The released plugin is still **v1.1.1** (Transfer only). A "TAKE (N)" button now sits beside "transfer"; it Ctrl+clicks the open stash tab's filter-matching items back into the bag, driven through the existing `TransferState` (all Ctrl-hold / watchdog / foreground-abort / cursor-restore guarantees shared). Needs in-game verification before release.
